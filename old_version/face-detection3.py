...
# MobileNet-SSD VOC Classes (20 classes)
CLASSES = [
...
# Load Neural Network & Haar Cascade Models
...
# ==========================================
# 🌐 FRONTEND WEB HUD UI (ANIMALS & OBJECTS BOOST)
# ==========================================
...
<head>
    ...
    <title>Cyber Vision HUD System v9</title>
    <style>
        body {
...
<body>
    ...
    <h1>[ CYBER VISION HUD v9 - ANIMAL & AI DETECTOR ]</h1>
    <div class="speed-bar" id="speedMeter">⚡ Engine: Ready | FPS: 0</div>
    ...
    <div id="status">Connecting to AI Neural Engine...</div>
    ...
    <script>
        ...
        async function startCamera(facingMode) {
            ...
            if (stream) {
                ...
                document.getElementById('status').innerText = "Camera Active | AI Animal & Face Detection Active";
        ...
        function captureAndProcess() {
            if (video.readyState === 4 && !isProcessing) {
                ...
                .then(data => {
                    ...
                    document.getElementById('speedMeter').innerText = `⚡ Latency: ${duration} ms | FPS: ${fps}`;
                    ...
                    if (data.detections && data.detections.length > 0) {
                        data.detections.forEach(item => {
                            if (item.w > 0) {
                                drawCyberHUD(item.x * scaleX, item.y * scaleY, item.w * scaleX, item.h * scaleY, item.label, item.confidence);
                            }
                        });
                    }
                })
        ...
        function drawCyberHUD(x, y, w, h, label, conf) {
            ...
            let displayText = `[ ${label.toUpperCase()} ${conf ? conf : ''} ]`;
            ctx.fillText(displayText, x, y - 10 > 15 ? y - 10 : y + 15);
        }

        setInterval(captureAndProcess, 120);
...
# ==========================================
# ⚙️ BACKEND SERVER ENGINE (DNN + CASCADE)
# ==========================================
...
def process_frame_with_ai(base64_str, cam_mode, torch_status, start_proc_time):
    try:
        ...
        # 1. Deep Learning MobileNet-SSD (For Birds/Poultry, Animals & Objects)
        if dnn_net is not None:
            ...
            for i in range(detections.shape[2]):
                ...
                if confidence > 0.35:  # Sensitivity threshold
                    ...
                    label_name = CLASSES[idx] if idx < len(CLASSES) else "object"
                    ...
                    detections_list.append({
                        ...
                        "w": int(bw),
                        "h": int(bh),
                        "label": label_name if label_name != "bird" else "bird/chicken",
                        "confidence": f"{int(confidence * 100)}%"
                    })

        # 2. Fallback to Face Cascade if no objects or for detailed face detection
        if len(detections_list) == 0 and face_cascade is not None:
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            faces = face_cascade.detectMultiScale(gray, scaleFactor=1.05, minNeighbors=3, minSize=(20, 20))
            for (x, y, fw, fh) in faces:
                detections_list.append({
                    "x": int(x), "y": int(y), "w": int(fw), "h": int(fh),
                    "label": "human face", "confidence": "99%"
                })
        ...
        return {
            ...
            "engine": "MobileNet-SSD Deep Learning + OpenCV Engine",
            "speed_meter": {
            ...
            "detections": detections_list if detections_list else [{"x": 0, "y": 0, "w": 0, "h": 0, "label": "none", "confidence": "0%"}]
        }
...
def launch_browser_immediately():
    ...
    print(f"\n{Colors.B_CYAN}[🚀] Launching Termux Browser to localhost:{PORT}...{Colors.RESET}")
    time.sleep(0.5)
...
def start_server():
    ...
    try:
        ...
        print(f"\n{Colors.B_GREEN}[✓] Multi-AI Engine Server Launched!{Colors.RESET}")
        launch_browser_immediately()
    except Exception as e:
...
def print_banner():
    C = Colors
    server_status = f"{C.B_GREEN}[ RUNNING ]{C.RESET}" if is_server_running else f"{C.B_RED}[ STOPPED ]{C.RESET}"
    banner = f"""
{C.B_CYAN}╔═════════════════════════════════════════════════════════════════════╗{C.RESET}
...
{C.B_CYAN}║{C.RESET} {C.B_YELLOW}[+] Mode                    :{C.RESET} {C.B_GREEN}Deep Learning Animal & Face HUD (v9){C.RESET} {C.B_CYAN}║{C.RESET}
{C.B_CYAN}╚═════════════════════════════════════════════════════════════════════╝{C.RESET}
"""
    print(banner)
...
def live_json_stream_mode():
    C = Colors
    print(f"\n{C.B_GREEN}[*] Entering Real-Time Live JSON Stream... (Press Ctrl+C to exit){C.RESET}\n")
    time.sleep(1)
    try:
        while True:
            clear_screen()
            print(f"{C.B_CYAN}═════════════════════════════════════════════════════════════════════{C.RESET}")
            print(f"{C.B_GREEN}⚡ [ REAL-TIME ANIMAL & OBJECT STREAM ]{C.RESET} | {C.B_RED}[Ctrl+C to Exit]{C.RESET}")
            print(f"{C.B_CYAN}═════════════════════════════════════════════════════════════════════{C.RESET}\n")
            ...
            if os.path.exists(DATA_FILE):
                try:
                    with open(DATA_FILE, 'r', encoding='utf-8') as f:
                        logs = json.load(f)
                        if logs:
                            print(f"{C.GREEN}{json.dumps(logs[-1], indent=4, ensure_ascii=False)}{C.RESET}")
                        else:
                            print(f"{C.B_YELLOW}[!] Waiting for frame data...{C.RESET}")
                except: pass
            else:
                print(f"{C.B_YELLOW}[!] Start server and open camera first.{C.RESET}")
            time.sleep(1)
...
{C.B_CYAN}├──────────────────────────────────────────────────────────────┤{C.RESET}
{C.B_CYAN}│{C.RESET} {C.B_GREEN}⚡ Latency / FPS    :{C.RESET} {C.B_YELLOW}{speed.get('processing_time')}{C.RESET} | {C.B_WHITE}{speed.get('fps_estimate')} FPS{C.RESET}
{C.B_CYAN}│{C.RESET} {C.B_CYAN}📷 Cam / Torch      :{C.RESET} {C.B_WHITE}{hw.get('camera_mode')}{C.RESET} | {C.B_YELLOW}Torch:{C.RESET} {hw.get('torch')}
{C.B_CYAN}│{C.RESET} {C.B_MAGENTA}🎯 Detections       :{C.RESET} {C.B_WHITE}{det.get('total_detected')} Targets Detected{C.RESET}
{C.B_CYAN}└──────────────────────────────────────────────────────────────┘{C.RESET}""")
        except: pass
    else: print(f"{C.B_RED}[!] Vault Empty.{C.RESET}")

def view_all_raw_json():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, 'r', encoding='utf-8') as f:
            print(f"{Colors.GREEN}{json.dumps(json.load(f), indent=4)}{Colors.RESET}")
    else: print(f"{Colors.B_RED}[!] Vault Empty.{C.RESET}")

def clear_vault():
    if os.path.exists(DATA_FILE):
        os.remove(DATA_FILE)
        print(f"\n{Colors.B_RED}[✓] Cleared!{Colors.RESET}")
    else: print(f"\n{Colors.B_YELLOW}[!] Already empty.{Colors.RESET}")

def show_menu():
    print_banner()
    C = Colors
    print(f" {C.B_YELLOW}[1]{C.RESET} {C.B_WHITE}Start Server & Launch Browser (v9 AI Boost){C.RESET}")
    print(f" {C.B_YELLOW}[2]{C.RESET} {C.B_WHITE}Stop Background Server{C.RESET}")
    print(f" {C.B_YELLOW}[3]{C.RESET} {C.B_GREEN}⚡ LIVE REAL-TIME JSON STREAM{C.RESET}")
    print(f" {C.B_YELLOW}[4]{C.RESET} {C.B_CYAN}View Structured Box Log{C.RESET}")
...
def main():
    ...
    while True:
        ...
        elif choice == '3': live_json_stream_mode()
        elif choice == '4': structured_box_view()
        elif choice == '5': view_all_raw_json()
        elif choice == '6': clear_vault()
        elif choice == '7':
            if is_server_running: stop_server()
            sys.exit()
        input(f"\n{Colors.B_CYAN}Press [{Colors.B_YELLOW}Enter{Colors.B_CYAN}] to continue...{Colors.RESET}")
        clear_screen()
