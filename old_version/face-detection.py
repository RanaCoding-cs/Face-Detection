...
<head>
    ...
    <style>
        ...
        h1 { color: #00ff66; text-shadow: 0 0 15px #00ff66; text-align: center; font-size: 18px; margin-bottom: 5px; }
        .speed-bar {
            background: #0d1527;
        ...
        #status { margin-top: 10px; color: #ffcc00; font-size: 12px; text-align: center; }
    </style>
</head>
<body>
    ...
    <script>
        ...
        async function toggleFlash() {
            ...
            if (capabilities.torch) {
                ...
                alert("Flashlight / Torch support not available on this lens!");
            }
        }
                ...
                .then(data => {
                    ...
                    if (data.face_coordinates && data.face_coordinates.length > 0) {
                        data.face_coordinates.forEach(coord => {
                            if (coord.w > 0) drawHUD(coord.x * 2, coord.y * 2, coord.w * 2, coord.h * 2);
                        });
                    }
                });
            }
        ...
        function drawHUD(x, y, w, h) {
            ...
            ctx.fillText(`TARGET LOCKED [OPENCV AI]`, x, y - 8);
        }
...
def launch_browser_immediately():
    ...
    #১. টার্মাক্সের স্ট্যান্ডার্ড ইউআরএল ওপেনার
    exit_code = os.system(f"termux-open-url {url}")
    
    #২. ব্যাকআপ হিসেবে পাইথন ওয়েবব্রাউজার
    if exit_code != 0:
        webbrowser.open(url)
